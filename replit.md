...
- **Isolated preview sandbox (manual only):** Set `LEGEND_BUCKS_SANDBOX=true` on the development API process and `VITE_LEGEND_BUCKS_SANDBOX=true` on the development Vite process. Both are development-only; the API refuses to start with the sandbox flag in any other `NODE_ENV`. Before opening the preview, run `NODE_ENV=development LEGEND_BUCKS_SANDBOX=true pnpm --filter @workspace/scripts run sandbox:seed` manually. Do not add this command to startup, deployment, migrations, or production jobs. It clones table *structures*, indexes/defaults and local serial sequences into `legend_bucks_sandbox` without rows or cross-schema FKs; the API pool has a sandbox-only search path. It is safe to re-run and verifies public row counts remain unchanged. Email delivery is simulated without credentials and object-storage routes (including image reads) return 403. Only sample employees are present; `aipulse@legendboats.com` is a simulated manager account in this schema, not the real employee. The preview quick-login and banner additionally require both Vite's development mode and the Vite sandbox flag. Clear both flags to return to normal operation. Do not run `drizzle-kit push` with sandbox mode enabled.
